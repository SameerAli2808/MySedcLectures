...
namespace FavSongs
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            List<Song> JerrySongs = Songs.Where(x => x.Tilte.StartsWith("B")).ToList();
            Jerry.FavoriteSongs = JerrySongs;

            List<Song> MariaSongs = Songs.Where(x => x.SongLength > 6).ToList();
            Maria.FavoriteSongs = JerrySongs;

            List<Song> JaneSongs = Songs.Where(x => x.Genre == Genre.Rock).ToList();
            Jane.FavoriteSongs = JaneSongs;

            List<Song> StefanSongs = Songs.Where(x => x.Genre == Genre.Hip_Hop && x.SongLength/60 < 3).ToList();
            Stefan.FavoriteSongs = StefanSongs;

            List<Person> Persons4 = FansArray.Where(x => x.FavoriteSongs.Count == 4).ToList();
