...
SELECT [FirstName]
FROM [dbo].[Teacher]
UNION all -- Needs to be with ALL in order to show the duplicates
SELECT [FirstName]
FROM [dbo].[Student]
...
SELECT [LastName]
FROM [dbo].[Teacher]
union --INTERSECT -- Needs to be Union Intersect will give only the names from both tables
SELECT [LastName]
FROM [dbo].[Student]
...
select DISTINCT t.[FirstName]+' '+t.[LastName] as [Teacher_Name]
from [dbo].[Teacher] t -- You cannot use Cross join because the cross join will return only values that exist in this 
-- case you need values that don't exist in the Grade table but exist in the Teacher table
left join [dbo].[Grade] g on t.[ID] = g.[TeacherID] 
